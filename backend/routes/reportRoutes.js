...
router.post('/upload', protect, upload.single('file'), async (req, res) => {
    try {
        // ✅ CORRECTED: Destructure 'reportType' from req.body instead of 'title' and 'notes'
        const { reportType } = req.body; 
        const userId = req.user.id;

        // You have no input for 'title' on the frontend, so you can set it programmatically here.
        ...
        const newReport = new Report({
            title,
            // ✅ CORRECTED: Pass the 'reportType' to the model instead of 'notes'
            reportType,
